const menu = [
 ...
 function foodCreator(name) {
   let item = `<div class="menu-items col-lg-6">
...
// Add All the Menus
 ...
 function addFoodsJapan() {
   section.innerHTML = ""
   const japaneseMenu = menu.filter(x => x.category == "Japan")
   japaneseMenu.map(element => {
     section.innerHTML += (foodCreator(element));
   });
 };
...
// Add Chinese Menu
  function addFoodsChina() {
    section.innerHTML = ""
    const chineseMenu = menu.filter(x => x.category == "China")
    chineseMenu.map(element => {
      section.innerHTML += (" " + foodCreator(element));
    });
  };
